...
async def _fetch_tmdb_data(query: str, api_key=None):
    """
    Core TMDB lookup: search â†’ fetch details â†’ build response dict.
...
async def get_movie_detailsx(query, id=False, file=None):
    ...
    try:
        ...
        if not data:
            logger.warning(f"TMDB returned no results for '{q}' â†’ switching to IMDb fallback")
            return await get_movie_details(q)
    except Exception as e:
        logger.error(f"TMDB direct call failed â†’ fallback IMDb: {e}")
        return await get_movie_details(q)
